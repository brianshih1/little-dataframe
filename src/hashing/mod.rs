...
pub fn series_to_hashes(series_list: &[Series], hasher: &RandomState) -> Vec<u64> {
...
pub fn hash_dataframes(frames: &[DataFrame], hasher: &RandomState) -> Vec<Vec<u64>> {
    POOL.install(|| {
        frames
            .par_iter()
            .map(|df| series_to_hashes(&df.columns, hasher))
    })
    .collect()
}
